...
/**
 * @brief Tclock definition default in NANOSECONDS
 */
uint16_t tclk = 488;
...
void Drv_ADS1299_initialize()
{
	...
	Drv_GPIO_init(); // For initialization GPIO pins, uncomment this line.
...
{
	//Recommended power up sequence requiers >Tpor (aprox. 32mS)

	WAIT1_Waitms(50); //Time until the power supplies have stabilized. See table 12, pag. 57 datasheet ADS1299.
	...
	WAIT1_Waitus(20);	//argument should be 18*tCLK or more, where tCLK = 1/2,048MHz = 488nSeg
	
	Drv_ADS1299_resetADS(BOARD_ADS);
    ...
    for(i=0; i<numChannels; i++)
    {
    	...
    	useInBias[i] = FALSE;    // keeping track of Bias Generation
...
void Drv_ADS1299_startADS()
{
	sampleCounter = 0;
	firstDataPacket = TRUE;
	Drv_ADS1299_RDATAC(BOARD_ADS); // enter Read Data Continuous mode
	//delay?
	Drv_ADS1299_START(BOARD_ADS);  // start the data acquisition
	//delay?
	isRunning = TRUE;
}
...
{ // deselect SPI slave
  switch(SS){
	case BOARD_ADS:
		...
		Drv_GPIO_FRDM_setup(CS_SPI,TRUE);
		break;
	case SLAVE_ADS:
...
uint8_t Drv_ADS1299_xfer(uint8_t _data)
{
	 ...
	 inByte = Drv_SSP_WriteRead(_data);
...
void Drv_ADS1299_WAKEUP(uint8_t targetSS)
{
	...
	//TODO: delayMicroseconds(3); //must wait 4 tCLK cycles before sending another command (see Datasheet, pag 35)
}
...
{
	Drv_ADS1299_csLow(targetSS);
	Drv_ADS1299_xfer(_RESET); //call to transfer function
	//TODO: delayMicroseconds(12); //must wait 18 tCLK cycles to execute this command (see Datasheet, pag 35)
	WAIT1_Waitus(9);	//argument should be 18*tCLK or more, where tCLK = 1/2,048MHz = 488nSeg to FRDM-K64F
	//Drv_ADS1299_csHigh(targetSS);	//this command used only you have an slave board
}
...
void Drv_ADS1299_SDATAC(uint8_t targetSS)
{
	...
	WAIT1_Waitus(2);	//argument should be 4*tCLK or more, where tCLK = 1/2,048MHz = 488nSeg
						//must wait 4 tCLK cycles before sending another command (see Datasheet, pag 35)
}
...
void Drv_ADS1299_RDATA(uint8_t targetSS)
{
	...
	if(targetSS == BOARD_ADS)
	{
		...
		for(i=0; i<8; i++)
		{
			...
			if((boardChannelData[i]&0x00800000) > 0) // convert 3 byte 2's compliment to 4 byte 2's compliment
			{
...
void Drv_ADS1299_writeOneChannelSettings(uint8_t N)
{
	...
	WAIT1_Waitus(2); //Wait 4*tCLK to send any command after
...
bool Drv_ADS1299_isDataAvailable()
{
	...
	return (!(Drv_GPIO_getPinState(0,0)));
}
...
void Drv_ADS1299_getChannelData(uint8_t *sampleCnt, uint8_t *data)
{
	*sampleCnt = sampleCounter; //copy sample counter value
	...
	for(i=0; i<48; i++)
	{
		...
		{
			data[i] = meanSlaveDataRaw[i - 24]; //store slave board data
		}
		else
		{
...
uint32_t Drv_ADS1299_getboardData(uint8_t channel_number, uint8_t targetSS){

	uint8_t i = channel_number - 1;

	if(targetSS==BOARD_ADS){
		return boardChannelData[i];
	}
	else{
		return slaveChannelData[i];
	}
}

uint8_t Drv_ADS1299_getGain(uint8_t channel_number, uint8_t targetSS){

	uint8_t gain = channelSettings[channel_number-1][GAIN_SET];
